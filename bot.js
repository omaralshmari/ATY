...
antispam(client, {
  ...
  time: 3600000, // عدد الوقت الي يجلس لين تسحب رتبة الميوت من الشخص الحسبة برمجية وليست كتابية 
});
...
client.on("guildMemberAdd", (member) => {
          ...
          .then(co => {
            text = co.first().content
              ...
              message.channel.send(`تم حفظ اقتراحك الرجاء انتضار الرد من قبل الاداره`)
                client.channels.get("475292101442142208").send(`${message.author.username}'s sug => ${text}`)
 ...
 client.on("roleCreate", rc => {
  const channel = rc.guild.channels.find("name", "𝗟𝗢𝗚𝗦") //تقدر تغير اسم الشات
  if(channel) {
  ...
  const channel = rd.guild.channels.find("name", "𝗟𝗢𝗚𝗦")
  if(channel) {
  var embed = new Discord.RichEmbed()
...
client.on("channelCreate",  cc => {
  const channel = cc.guild.channels.find("name", "𝗟𝗢𝗚𝗦")
  if(channel) {
  var embed = new Discord.RichEmbed()
  ...
  const channel = dc.guild.channels.find("name", "𝗟𝗢𝗚𝗦")
  if(channel) {
  var embed = new Discord.RichEmbed()
...
client.on('guildMemberRemove', member => {
    ...
    const channel = member.guild.channels.find('name', '𝗟𝗢𝗚𝗦');
    if (!channel) return;
...
client.on('messageDelete', message => {
    ...
    const channel = message.guild.channels.find('name', '𝗟𝗢𝗚𝗦');
    if (!channel) return;
...
const type = require('./lgz/typePTS.json');
...
.addField("**:diamond_shape_with_a_dot_inside: Support ™ **","** **")
 .addField("** مبرمجي مبرمج الكود :wrench: **","**™ » <@431150885549113344>. **")
