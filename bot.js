...
var title = args[1]
         ...
         if(!isNaN(args)) return message.channel.send('✖| **The Time Be Nambers `` Do the Commend Agin``**');
                           let giveEmbed = new Discord.RichEmbed()
                  .setAuthor(message.guild.name, message.guild.iconURL)
                  .setDescription(${title} \nReact Whit :tada: To Enter! \nTime remaining: Minutes :${duration / 60000})
                  .setFooter(message.author.username, message.author.avatarURL);

                  message.channel.send(' ✔ **Giveaway Created** ✔' , {embed: giveEmbed}).then(m => {
                      message.delete();
                      m.react('🎉');
                     setTimeout(() => {
                     ...
                     m.edit(' :tada: GIVEAWAY ENDED :tada:' , {embed: endEmbed});
                     },args * 60000);
                   });
          }
});
